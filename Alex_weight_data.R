...
ggplot(data=Alex_Borger_Weight_Data[23:277,], aes(x=Date,y=Weight))+
...
for (x in c(7:240)) {
  ...
  if (Alex_Borger_Weight_Data$Weight[x] < prev){
    count <- count + 1
  }
  else {
  ...
  if (Alex_Borger_Weight_Data$Weight[x] < currentlev){
    print(Alex_Borger_Weight_Data[x,])
    currentlev <- currentlev - 10
  }
  prev <- Alex_Borger_Weight_Data$Weight[x]
